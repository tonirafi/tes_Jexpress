...
class UploadBean(
    val uuid: Int,
    val imgPath: String,
    private var listener: ImageUploadingListener?
) : UploadCallback {
    ...
    fun getProgress() =
        if (totalSize > 0) String.format("%.1f", currSize.toFloat() / totalSize * 100) else "0.0"
...
//        val imgSuffix = if (imgPath.lastIndexOf('.') == -1) ".jpg" else imgPath.substring(imgPath.lastIndexOf('.'))
    ...
    private fun canNotify(requestId: String?): Boolean =
