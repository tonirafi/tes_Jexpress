...
class UploadImageProgress : BaseBean() {
    ...
    override fun toString() =
        "UploadImageProgress{uuid='$uuid', path=$path, currentSize=$currentSize ,totalSize=$totalSize}"
    ...
    fun getProgress() =
        if (totalSize > 0) String.format("%.1f", currentSize.toFloat() / totalSize * 100) else "0.0"
