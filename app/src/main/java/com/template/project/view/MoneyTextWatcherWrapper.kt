 ...
 *   描述：
 */
class MoneyTextWatcherWrapper(val goalMoney: EditText, val goalMoneySuffix: TextView, private val textWatcher: TextWatcher) : TextWatcher {
    ...
    override fun onTextChanged(s: CharSequence?, start: Int, before: Int, count: Int) {
        ...
        preCleanString = if (preCleanString == cleanString) {
            val tmp = s.removeRange(start - before, start - before + 1).replace("[.,]".toRegex(), "").toLongOrNull()
                    ?: 0
        ...
        val sel = if (delta < 0) goalMoney.selectionStart + delta + 1 else goalMoney.selectionStart + delta - 1
        ...
        LogUtil.logE("TXT", "onTextChanged= sel=$sel ;selectionStart=${goalMoney.selectionStart} ;selection=$selection")
